...
//0706 fixing zadiac death loop problem
...
function extractPartialMonthDay(text: string): { month: string; day: string } | undefined {
  const normalized = text.trim();

  // 支援「說錯應該是3/12」「那2/13呢」「1/1呢」這種省略年份的追問。
  // 若前面已經有 birthDate，App 會沿用前一次年份補成 YYYY-MM-DD。
...
function shouldUseZodiacCheck(text: string, state: FortuneProfileState): boolean {
  ...
  const hasBirthDateNow = Boolean(extractBirthDate(text));
  const hasPartialDateNow = Boolean(extractPartialMonthDay(text));
  const hasZodiacNow = Boolean(extractZodiac(text));
  const hasAlgorithmSwitch = Boolean(extractAlgorithm(text));
  ...
  if (hasBirthDateNow && hasZodiacNow) return true;
  if (hasBirthDateNow && isQuestion) return true;
  if (hasBirthDateNow && isChallenge) return true;
  if (hasBirthDateNow && Boolean(state.userZodiac)) return true;

  if (hasPartialDateNow && Boolean(state.birthDate) && isQuestion) return true;
  if (hasPartialDateNow && Boolean(state.birthDate) && isChallenge) return true;
  if (hasPartialDateNow && Boolean(state.birthDate) && Boolean(state.userZodiac)) return true;
...
function lookupZodiacByBirthDate(input: {
  birthDate: string;
  algorithm: ZodiacAlgorithm;
}): ZodiacCheckPayload {
  const birthDate = input.birthDate;
...
function AppContent() {
  ...
  function updateFortuneProfileFromTranscript(transcript: string) {
    const algorithm = extractAlgorithm(transcript);
    const birthDate = extractBirthDate(transcript);
    const partialMonthDay = extractPartialMonthDay(transcript);
    const zodiac = extractZodiac(transcript);
    ...
    if (birthDate) {
      fortuneProfileRef.current.birthDate = birthDate;
    } else if (partialMonthDay && fortuneProfileRef.current.birthDate) {
      const existingYear = fortuneProfileRef.current.birthDate.slice(0, 4);
      fortuneProfileRef.current.birthDate = `${existingYear}-${partialMonthDay.month}-${partialMonthDay.day}`;
    }
  ...
  function handleUserUtteranceForResponse(transcript: string, source: "text" | "audio" | "ptt" | "fallback") {
    ...
    try {
      const lunarYear = extractLunarNewYearYear(cleaned);
      const isQuestion = asksZodiacQuestion(cleaned);
      const isChallenge = isZodiacChallenge(cleaned);
      ...
      // 使用者問「屬什麼」、追問「那 2/13 呢」、或質疑「不是屬牛嗎」時，
      // 優先用目前狀態中的生日查出正確生肖，不讓模型自己硬算。
      if ((isQuestion || isChallenge) && fortuneProfileRef.current.birthDate) {
        const payload = lookupZodiacByBirthDate({
          birthDate: fortuneProfileRef.current.birthDate,
          algorithm: fortuneProfileRef.current.algorithm,
        });
        ...
        postLog({
          role: "system",
          content: `[ZODIAC_CHECK_RESULT] ${JSON.stringify(payload).slice(0, 1200)}`,
          eventId: `zodiac_birthdate_lookup_${Date.now()}_${Math.random().toString(36).slice(2)}`,
        });

        createAssistantAudioResponse(
          "(trigger response with zodiac birthdate lookup)",
          buildZodiacResponseInstructions(payload)
        );
        return;
      }
...
- 此解籤模式禁止呼叫任何工具或外部搜尋；需要精準生肖或農曆分界時，一律由 App.tsx 的本地規則驗證結果提供。
- 如果使用者語音聽起來不清楚、內容不完整、像背景音，或和目前對話脈絡明顯不相關，不要直接推銷或回答；請先說：「我剛剛沒有聽清楚，可以再說一次嗎？」
- 如果轉錄看起來是英文短句，例如 Yeah、Why、Bye、way over there，但前後脈絡主要是中文，請優先判斷可能是誤辨識，先確認，不要直接結束對話或切到英文回覆。
- 若本回合 response instructions 內含 [ZODIAC_CHECK_RESULT]，生肖、農曆新年分界日、生肖年、是否一致都必須完全依照該結果，不得自行查表或改日期。`;
    ...
    sendClientEvent(sessionUpdateEvent, "agent.no_tools + zodiac_router");
  };
